...
def main():
    """
    Generate a data config (json) based on predefined parameters
    """
    ...
    for phase in range(n_classes):
        if distribution == "gamma":
            n_peaks = np.round((rng.gamma(1.2, 1.2) + 2)).astype(
                int
            )  # favor less peaks
        ...
        peak_positions = rng.integers(boundary, n_datapoints - boundary, n_peaks)[
            :max_peaks
        ]
