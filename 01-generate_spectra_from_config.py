...
def main():
    ...
    for i, phase in enumerate(tqdm(spectra.keys())):
        ...
        if int(phase) in [164, 281, 219, 347]:  # exclude classes with overlapping ints
            new_hi_hi = np.array([f + 0.025 if f != 1.0 else f for f in peak_heights])
            new_hi_lo = np.array([f - 0.025 if f != 1.0 else f for f in peak_heights])
        else:
            new_hi_hi = np.array(
                [f + test_var if f != 1.0 else f for f in peak_heights]
            )
            new_hi_lo = np.array(
                [f - test_var if f != 1.0 else f for f in peak_heights]
            )
